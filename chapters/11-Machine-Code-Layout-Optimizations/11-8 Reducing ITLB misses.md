...
BOLT provides an option to automatically use huge pages for hot code based on
profile data. The benefits of this approach is that only the small portion of
the code is mapped to the huge pages and the number of required huge pages is
minimized, and as a side effect, page fragmentation is reduced.
The implementation leverages Linux Transparent Huge Pages (THP) for requesting
huge pages at runtime.
[TODO]: provide the option/documentation.
