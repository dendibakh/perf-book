...
![Screenshot captured with ETWController and WPA Profiling Data.](../../img/perf-tools/ETWControllerScreenshot_WPA_Combined.png){#fig:ETWController_WPA width=80% }

On the screenshot @fig:ETWController_WPA you see on top the double click event as it is saved by ETWController. Below is the recorded ETW data opened in Windows Performance Analyzer (WPA).
The WPA view is divided into three parts: *CPU Usage (Sampled)*, *Generic Events* and *CPU Usage (Precise)*. Let's understand the difference. The upper graph *CPU Usage (Sampled)* is useful to see where CPU is spent, similar to the hotspots view in other profiling tools. CPU sampling works by taking a stack trace of all threads which are currently executing at a regular (sampling) interval. When you sum up all stack traces you get accurate CPU consumption data of your methods consuming most CPU (hot spots). This is what VTune is doing and much more by adding additional CPU specific counters to give hints how you can restructure your code to work around CPU/Memory bottlenecks.
...
A combined CPU Sampling/CSwitch view would give accurate per method CPU usage data (originating from sampling data) while the exact wait/ready timings would be generated by Context Switch traces. 
...
Mystery solved! The 1.068s delay in starting our process is due to Defender Scanning activty. When we drill into the call stack we will find that CreateProcess is delayed in the kernel by WDFilter.sys which will block process start until the potentially malicious file contents are scanned. Antivirus can intercept everything and you will get funny performance issues no one can root cause unless you get a full kernel view like with ETW. 
...
If you group via the Column editor in CPU Usage Precise
