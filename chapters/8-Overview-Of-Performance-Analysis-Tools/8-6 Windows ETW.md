...
I suggest to structure it as follows:
1. Show the screenshot
2. Describe the panels
3. Explain the mystery (!)
  - show the double-click screenshot
4. Discuss stack tags
5. Custom WPA profiles
6. 
...
[TODO:] I'm curious, what benefits will a combined view bring?

[TODO:] Please explain the `CPU Usage (Sampled)` panel.
E.g. "We can see `explorer.exe` is active around timestamps `34.95` and `35.1`, but then explorer wasn't doing much work and the application didn't start. Instead, MsMpEng takes over the execution up until the time `35.7`. (What happens after that?).
...
[TODO:] Please explain the `CPU Usage (Precise)` panel similar to the `CPU Usage (Sampled)` panel.
...
[TODO:] Please describe what the columns `CPU ms` and `Wait ms` mean.
