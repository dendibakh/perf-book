...
**Loop Invariant Code Motion (LICM)**: an expression whose value never changes across iterations of a loop is called a *loop invariant*. Since its value doesn't change across loop iterations, we can move a loop invariant expression outside of the loop. We do so by storing the result in a temporary variable and use it inside the loop (see [@lst:LICM]). All decent compilers nowadays successfully perform LICM in the majority of cases.
...
The primary benefit of loop unrolling is to perform more computations per iteration. At the end of each iteration, the index value must be incremented, tested, and the control is branched back to the top of the loop if it has more iterations to process. This work is commonly reffered to as "loop overhead" or "loop tax", and it can be reduced. For example, by unrolling the loop in [@lst:Unrol] by a factor of 2, we reduce the number of executed compare and branch instructions by half.

[TODO]: loop remainder
[TODO]: unrolling too much could have negative consequencies due to code bloat
...
Loop Blocking is a widely known method of optimizing GEneral Matrix Multiplication (GEMM) algorithms. It enhances the cache reuse of memory accesses and improves both memory bandwidth and memory latency of an algorithm.
...
[TODO]: there is a problem with this example. If you change a call to "arbitraryFunction(a)" instead of "strlen(a)", then the opportunity for optimization (even with use of the "restrict" keyword) would disappear. 
