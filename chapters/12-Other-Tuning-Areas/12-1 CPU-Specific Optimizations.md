...
The code looks very logical; it uses fused multiply add instruction to compute product of one element of `a` (in `xmm1`) and then accumulates the result in `xmm0`. The problem is: there is a data dependency over `xmm0`. A processor cannot issue a new `vfmadd231ss` instruction until the previous one has finished since `xmm0` is both an input and an output of `vfmadd231ss`. Performance of this loop is bound by FMA latency, which in Intel's Alderlake equals to 4 cycles.
