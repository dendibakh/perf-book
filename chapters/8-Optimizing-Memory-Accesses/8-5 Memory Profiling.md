...
The solid line (MEM) tracks the number of bytes accessed during each interval of 1B instructions. Here, we don't count how many times a certain memory location was accessed. That is, if a memory location was loaded twice during an interval, we count the touched memory only once. For the same reason, we cannot aggregate time intervals. For example, we know that Blender was touching roughly 20MB every interval. We cannot aggregate its 150 consecutive intervals and say that the total memory footprint was `150 * 20MB = 3GB`, because the same memory location could be accessed in adjacent 1B instruction intervals. It would be true only if the program never repeated memory accesses.

The dashed line (NEW) tracks the size of the new data accessed since the start of the program. Here, we count the number of bytes accessed during each 1B instructions interval that have never been touched before by the program. Aggregating all the NEW intervals (cross-hatched area under that dashed line) gives us the total memory footprint for a program. Here are the numbers for our benchmarks: Clang C++ compilation (487MB); Stockfish (188MB); PSPNet (1888MB); Blender (149MB). Keep in mind, some of these memory locations are accessed multiple times, so the overall pressure on the memory subsystem is higher.
