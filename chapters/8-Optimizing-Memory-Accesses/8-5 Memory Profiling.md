...
Memory footprint is somewhat an overloaded term. Some resources use it to describe total virtual memory used by a process. However, in this section, we define *memory footprint* as the number of bytes a process accesses during a period of time, measured, for example, in MB per second. This definition enables us to describe the intensity of memory operations more accurately, and observe how it changes over time.

### Case Study: Memory Footprint of Four Workloads {.unlisted .unnumbered}

In this case study we will analyze the memory footprint of four workloads: Blender ray tracing, Stockfish chess engine, Clang++ compilation, and AI_bench PSPNet segmentation. We hope that this study will give you an intuition on memory intensity of some real-world applications. We collect memory footprint using the Intel SDE (Software Development Emulator) tool using the method described on the Easyperf [blog](https://easyperf.net/blog/2024/02/12/Memory-Profiling-Part3)[^6] with intervals of one billion instructions. Figure @fig:MemFootCaseStudyFourBench shows the memory footprint of four selected workloads. 
...
The solid line (MEM) tracks the number of bytes accessed during each interval of 1B instructions. Here, we don't count how many times a certain memory location was accessed. That is, if a memory location was loaded twice during an interval, we count the touched memory only once. For the same reason, we cannot aggregate time intervals. For example, we can see that, on average, the Blender benchmark touches roughly 20MB every interval. We cannot aggregate its 150 consecutive intervals and say that the total memory footprint of Blender was `150 * 20MB = 3GB`, because the same memory location could be accessed in two adjacent 1B instruction intervals. It would be true only if a program never repeats memory accesses across intervals.

The dashed line (NEW) tracks the size of the new data accessed every interval since the start of the program. Here, we count the number of bytes accessed during each 1B instructions interval that have never been touched before by the program. Aggregating all the NEW intervals (cross-hatched area under that dashed line) gives us the total memory footprint for a program. Here are the total memory footprint numbers for our benchmarks: Clang C++ compilation (487MB); Stockfish (188MB); PSPNet (1888MB); Blender (149MB). Keep in mind, some of these memory locations are accessed multiple times, so the overall pressure on the memory subsystem is higher.
